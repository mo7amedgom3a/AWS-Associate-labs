...
# IAM User Access Testing Script
# This script tests the access permissions of each IAM user created in setup.sh
...
echo "=== IAM User Access Testing Script ==="
...
test_user_access() {
    ...
    local key_output=$(aws iam create-access-key --user-name ${user} --output text)
    local access_key=$(echo "${key_output}" | awk '{print $2}')
    local secret_key=$(echo "${key_output}" | awk '{print $4}')
